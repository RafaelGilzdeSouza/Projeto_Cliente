...
if(strlen($produtos) > 1){ //a busca é valida? (sim)
    $resultado_prod = "SELECT * 
                       FROM tb_produtos 
                       WHERE descricao LIKE '%$produtos%' OR grupo LIKE '%$produtos%' 
                       ORDER BY descricao ASC";
    $resultado_busca = $mysqli->query($resultado_prod) or die("Falha na execução do código SQL: " . $mysqli->error);
    
    if ( ($resultado_busca) AND ($resultado_busca->num_rows != 0) ) { //foi retornado algum produto do banco? (sim)
    ...
    }else{ //foi retornado algum produto do banco? (nao)
        echo utf8_encode('<p class="lead fw-normal text-white-50 mb-0" >Produto nao encontrado :(</p>');}
}else{ //a busca é valida? (nao, entao mostre os produtos em promocao)
    $resultado_prod = "SELECT * 
                       FROM tb_produtos
                       WHERE promocao = 1";
    $resultado_busca = $mysqli->query($resultado_prod) or die("Falha na execução do código SQL: " . $mysqli->error);
